...
def calculate_ps_for_presentations(lfp_slice, time_range, sf=1250):
    f_all, s_all = [], []
    ...
    return np.array(f), np.array(s_all)
...
def run(cache, probe_list, session_id, VELOCITY_THRESHOLD, window_range, sf, output_folder):
    ...
    _, s1 = calculate_ps_for_presentations(aligned_lfp, window1)
    f, s2 = calculate_ps_for_presentations(aligned_lfp, window2)
...
if __name__ == '__main__':
    output_folder = Path.home() / 'Desktop' / 'disk2' / 'lfp_power_spectra_dump_0-1_-1-2'
